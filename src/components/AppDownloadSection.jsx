import React, { useState, useEffect } from 'react';
import { Check, ShoppingCart, Truck, Shield } from 'lucide-react';
import phone from '../assets/phone.jpg';

const AppDownloadSection = () => {
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    setIsVisible(true);
  }, []);

  // 🔥 DOWNLOAD HANDLER (IMPORTANT)
  const handleAppDownload = () => {
    const link = document.createElement('a');
    link.href = '/app-release.apk'; // must be in public folder
    link.download = 'TheQuickPoint-App.apk';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const highlights = [
    { icon: ShoppingCart, text: 'Easy and quick ordering' },
    { icon: Truck, text: 'Freshly prepared products' },
    { icon: Shield, text: 'Reliable and hygienic delivery' },
    { icon: Check, text: 'Trusted local dairy brand' }
  ];

  return (
    <section className="bg-black py-16 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">

          {/* LEFT CONTENT */}
          <div
            className={`transition-all duration-1000 transform ${
              isVisible ? 'translate-x-0 opacity-100' : '-translate-x-12 opacity-0'
            }`}
          >
            <h2 className="text-white text-4xl sm:text-5xl lg:text-6xl font-bold mb-6">
              Download the <span className="text-[#FF8C00]">THE QUICK POINT</span> App
            </h2>

            <p className="text-[#F5F5F5] text-xl sm:text-2xl mb-6">
              Freshness, purity, and authentic dairy products — delivered with speed.
            </p>

            <p className="text-[#F5F5F5] text-base sm:text-lg mb-10 max-w-[520px]">
              Order pure desi ghee and fresh curd products effortlessly using our mobile app.
              Enjoy trusted quality and fast delivery.
            </p>

            {/* HIGHLIGHTS */}
            <div className="grid sm:grid-cols-2 gap-4 mb-10">
              {highlights.map((item, index) => (
                <div key={index} className="flex items-start gap-3">
                  <div className="p-2 rounded-lg">
                    <item.icon className="w-5 h-5 text-[#0B8F00]" />
                  </div>
                  <span className="text-[#F5F5F5] text-sm sm:text-base">
                    {item.text}
                  </span>
                </div>
              ))}
            </div>

            {/* DOWNLOAD BUTTONS */}
            <div className="flex flex-wrap gap-4">

              {/* 🔥 GOOGLE PLAY DOWNLOAD */}
              <button
                onClick={handleAppDownload}
                className="cursor-pointer bg-white text-black px-6 py-3 rounded-lg font-semibold flex items-center gap-3 
                           hover:scale-105 hover:shadow-2xl hover:shadow-[#FF8C00]/20 transition-all duration-300
                           active:scale-95"
              >
                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z" />
                </svg>
                <div className="text-left">
                  <div className="text-xs opacity-70">GET IT ON</div>
                  <div className="text-sm font-bold">Google Play</div>
                </div>
              </button>

              {/* APP STORE (COMING SOON) */}
              <button
                disabled
                className="cursor-not-allowed bg-gray-300 text-gray-600 px-6 py-3 rounded-lg font-semibold flex items-center gap-3 opacity-70"
              >
                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.71,19.5C17.88,20.74 17,21.95 15.66,21.97C14.32,22 13.89,21.18 12.37,21.18C10.84,21.18 10.37,21.95 9.1,22C7.79,22.05 6.8,20.68 5.96,19.47C4.25,17 2.94,12.45 4.7,9.39C5.57,7.87 7.13,6.91 8.82,6.88C10.1,6.86 11.32,7.75 12.11,7.75C12.89,7.75 14.37,6.68 15.92,6.84C16.57,6.87 18.39,7.1 19.56,8.82Z" />
                </svg>
                <div className="text-left">
                  <div className="text-xs opacity-70">Coming Soon on</div>
                  <div className="text-sm font-bold">App Store</div>
                </div>
              </button>

            </div>
          </div>

          {/* RIGHT PHONE MOCKUP */}
          <div
            className={`relative transition-all duration-1000 transform ${
              isVisible ? 'translate-x-0 opacity-100' : 'translate-x-12 opacity-0'
            }`}
          >
            <div className="relative mx-auto max-w-[300px] lg:max-w-[400px]">
              <div className="absolute inset-0 bg-gradient-to-br from-[#FF8C00] to-[#0B8F00] rounded-[3rem] blur-3xl opacity-30 animate-pulse"></div>

              <div className="relative bg-gradient-to-br from-gray-800 to-black p-3 rounded-[3rem] shadow-2xl">
                <div className="bg-white rounded-[2.5rem] overflow-hidden">
                  <img
                    src={phone}
                    alt="App Screenshot"
                    className="w-full h-full object-cover aspect-[9/19]"
                  />
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>
    </section>
  );
};

export default AppDownloadSection;
